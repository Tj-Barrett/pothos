...
void align(file_info *file_info,
              vector<atom_info> *step_info,
              atom_avs  *atom_avs,
              int type, int leg, int k, int verbose )
  {
    if (type < 0){
      // same as in legendre file, figure out how to use it
      ...
      for (int poly=0; poly<polymers; poly++){
        ...
        for (int mono=0; mono<monomers; mono++){
          ...
          verho = _verho_(atom_range);

          (*step_info)[ind].align   =verho[0];
          (*step_info)[ind].dx      =verho[1];
          (*step_info)[ind].dy      =verho[2];
          (*step_info)[ind].dz      =verho[3];

          (*atom_avs).align    +=verho[0];
          (*atom_avs).dx       +=verho[1];
          (*atom_avs).dy       +=verho[2];
          (*atom_avs).dz       +=verho[3];
        }
...
// processing
void align_processing(string filename,
                string outFile,
                file_info *file_info,
                int type,
  ...
  while(std::getline(infile,line)){
    ...
    // last line triggers writing header line and data
      ...
      // align compute
      align(file_info, &step_info, &atom_avs, type, leg, k, verbose);
      // cluster it
      cluster(file_info, &step_info, &atom_avs, type, leg, k, verbose);
      // write data
...
//                                                                            //
//============================================================================//
int legendre_compute(string inFile,
                     string outFile,
                     int leg,
                     int k,
                     int verbose) {
...
// void syntax()
// {
//   printf("  legendre [-pkv] input_file\n");
//   printf("    -p  Legendre type (P2,P4,P6).\n");
//   printf("    -k  Interval length. 1 is chord vector only. \n");
//   printf("    -v  Verbosity level (0,1,2,3).\n");
// }
//
// //============================================================================//
// //                                                                            //
// // usage()    Print a brief instruction for using legendre.                   //
// //                                                                            //
// //============================================================================//
//
// void usage()
// {
//   printf("pothos++ legendre\n");
// }
